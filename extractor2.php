<?php
    ...
    $extractExceptTables = function($tableList, $extractToFile, $verbose=true) use ($reader, $writer) {
    ...
    $extractExceptDataInTables = function($tableList, $extractToFile, $verbose=true) use ($reader, $writer) {
        ...
        foreach($reader() as $k=>$line) {
            if (
                ...
                $table = trim($regs[1]);
                if (in_array($table, $tableList) && preg_match('~INSERT[^`;+]INTO[^`;+]`([^`]+)`~Usi', $line)) {
                    $writeMarker = false;
                } else {
                    $writeMarker = true;
                }
    ...
    switch($action) {
        ...
        case '--extract-except':
            ...
            $extractExceptTables($extractTableList, $extractToFile);
            
            echo 'Complete!'.PHP_EOL;
            
        break;
        
        case '--extract-except-data':
            ...
            $extractExceptDataInTables($extractTableList, $extractToFile);
